...
v2: 
  - Checkpoint/EarlyStopping monitorizan val_loss (no val_lm)
  - Artifacts se guardan desde el MEJOR checkpoint (no el último epoch)
  - Safe LoRA loading para compatibilidad con versiones antiguas de PEFT
"""
...
def load_lora_adapter_safe(base_model, lora_path: str):
    ...
    Estrategia:
      1) Leer adapter_config.json
      2) Conservar SOLO las claves aceptadas por LoraConfig.__init__() en ESTE entorno
         + unas pocas claves "metadata" que PeftModel suele necesitar.
      3) Crear un tempdir con el config filtrado y los pesos originales
      4) PeftModel.from_pretrained(base_model, tmpdir)
    """
    cfg_path = os.path.join(lora_path, "adapter_config.json")
    with open(cfg_path, "r") as f:
        cfg = json.load(f)
    
    # Claves aceptadas por TU versión de PEFT
    sig = inspect.signature(LoraConfig.__init__)
    allowed_init = set(sig.parameters.keys()) - {"self"}
    
    # Algunas claves de metadata que a veces aparecen/ayudan (si existen)
    allowed_meta = {
        "peft_type",
        "auto_mapping",
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # Copiar pesos y demás ficheros del adapter
...
class LitCVAE(pl.LightningModule):
    ...
    def compute_ablation(self, batch, z, lm_loss_real):
        with torch.no_grad():
            ...
            return delta_shuffle, delta_zero, lm_shuffled, lm_zero
        ...
        if stage == "val":
            ...
            if ablation_every > 0 and (self.current_epoch % ablation_every == 0):
                if not getattr(self, "_ablation_done_this_epoch", False):
                    dsh, dz0, lm_shuf, lm0 = self.compute_ablation(batch, z, lm_loss)
...
def main():
    ...
    print("=" * 60)
    print("Phase 1C CVAE (v2: val_loss + best checkpoint artifacts)")
    print("=" * 60)
    ...
    # =========================================================
    # CAMBIO 1: Monitorizar val_loss en vez de val_lm
    # =========================================================
    ...
    trainer = Trainer(
        ...
        callbacks=[
            ckpt_cb,
            EarlyStopping("val_loss", patience=args.patience, mode="min"),  # CAMBIO: val_loss
    ...
    # =========================================================
    # CAMBIO 2: Cargar MEJOR checkpoint antes de guardar artifacts
    # =========================================================
    ...
    if best_ckpt_path and os.path.isfile(best_ckpt_path):
        ...
        # Cargar state_dict del mejor checkpoint con strict=False para tolerancia
        if "state_dict" in best_ckpt:
            ...
            if best_score_f is not None:
                ...
                print("[INFO] Loaded best model state")
        else:
    ...
    # Save artifacts (ahora desde el mejor checkpoint)
    print("\n[INFO] Saving artifacts from BEST checkpoint...")

    model.tok.save_pretrained(os.path.join(art_dir, "tokenizer"))

    # Guardar LoRA adapter con manejo de compatibilidad PEFT
    ...
    metadata = {
        ...
        # Checkpoint selection info
        "checkpoint_monitor": "val_loss",
        "best_checkpoint": best_ckpt_path,
        "best_val_loss": best_score_f,
        # Loss weights (para saber cómo se compuso val_loss)
        "loss_weights": {
