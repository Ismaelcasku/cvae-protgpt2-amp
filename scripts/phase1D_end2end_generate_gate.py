...
v2 (Gate support):
- Soporta modelos con gate escalar
- Carga gate.pt si existe en artifacts
- Aplica g = sigmoid(gate(mu)) durante generación
- Generación eficiente con KV-cache
- predict_mic vectorizado si heads disponibles
"""
...
class CVAEGenerator:
    def __init__(self, artifacts_dir: str, device: str = "cuda"):
        ...
        print(f"[INFO] Use gate: {self.meta.get('use_gate', False)}")
        ...
        # Gate (optional)
        self.use_gate = bool(self.meta.get("use_gate", False))
        self.gate = None
        gate_path = os.path.join(artifacts_dir, "gate.pt")
        if self.use_gate and os.path.exists(gate_path):
            gate_ckpt = torch.load(gate_path, map_location="cpu")
            self.gate = nn.Linear(self.latent_dim, 1)
            self.gate.load_state_dict(gate_ckpt["state_dict"])
            self.gate.to(self.device).eval()
            print("[INFO] Loaded gate.pt")
        elif self.use_gate:
            print("[WARN] use_gate=True in metadata but gate.pt not found, ignoring gate")
            self.use_gate = False
    ...
    def compute_gate(self, mu: torch.Tensor) -> torch.Tensor:
        """Compute gate value. Returns [B, 1] in (0, 1)."""
        if not self.use_gate or self.gate is None:
            return None
        return torch.sigmoid(self.gate(mu))
    ...
    def generate_batch(
        ...
        Args:
            ...
            bacteria: bacteria condition
            mu_for_gate: if provided and gate is enabled, use this for gate computation
                         (typically mu when z is perturbed around mu)
        """
        ...
        if self.use_gate and self.gate is not None:
            # Use mu_for_gate if provided, else use z (for prior sampling, z acts as "mu")
            mu_g = mu_for_gate if mu_for_gate is not None else z
            g = self.compute_gate(mu_g)  # [B, 1]
            prefix_emb = prefix_emb * g.view(B, 1, 1)
        ...
        # Initial forward
        prompt_emb = self.dec.get_input_embeddings()(prompt_ids_t)
        inputs_embeds0 = torch.cat([prefix_emb, prompt_emb], dim=1)
        ...
        generated = prompt_ids_t.clone()
        finished = torch.zeros(B, dtype=torch.bool, device=self.device)

        next_logits = out0.logits[:, -1, :]
    ...
    def predict_mic_batch(self, sequences: List[str], bacteria: str) -> Tuple[Optional[List[float]], Optional[List[float]]]:
        ...
        mu = self.encode_mu(sequences)
        z = mu  # Use mu for prediction (deterministic)
        ...
        input_ids = torch.tensor(ids_pad, device=self.device, dtype=torch.long)
        attn = torch.tensor(attn_tok, device=self.device, dtype=torch.long)
        ...
        idx = (self.prefix_len + lengths - 1).to(torch.long)
        h_T = last[torch.arange(B, device=self.device), idx]

        mic_pred = self.reg_head(h_T).squeeze(-1)
        active_prob = torch.sigmoid(self.cls_head(h_T).squeeze(-1))
...
def main():
    ...
    p.add_argument("--max_tries_factor", type=int, default=40)

    p.add_argument("--reference_csv", type=str, default=None)
    ...
    for bac in bacteria_list:
        ...
        while len(unique) < target_n and tries < max_tries:
            ...
            if args.mode == "prior":
                ...
                # For prior, z acts as mu for gate (no separate mu)
                ...
                # Use original mu for gate (more stable than perturbed z)
                mu_for_gate = mu.repeat(batch_n, 1)

            else:  # interpolate
                if not args.seed_seq or not args.seed_seq2:
                    raise ValueError("--seed_seq and --seed_seq2 required for mode=interpolate")
                ...
                mu_for_gate = z  # interpolated points are like mu
        ...
        if len(unique) < target_n:
            print(f"[WARN] Only generated {len(unique)}/{target_n} unique for {bac}")
    ...
    print(f"\n[OK] Saved: {args.output} | rows={len(out_df)} | unique={out_df['sequence'].nunique()}")
